...
for i in range(len(BOT_GROUP)):
    list_bot_name.append({"stt": i, "bot": BOT_GROUP[i].get_me().first_name})
...
def io_check_telegram(message):
    ...
    is_url, is_member, gr_id, gr_title, gr_username = get_channel_info(
        link=message["link"], channel_id=group_id
    )

    room_id = message["room"]
    socketio.emit(
        "check_tele_result_" + room_id,
        {
            "is_url": is_url,
            "is_member": is_member,
            "group_id": gr_id,
...
def io_confirm_licence(message):
    result = request_confirm_licence(int(message["ordertime"]), int(message["licence"]))
...
def io_send_message(message):
    room_id = message["room"]
    result = request_send_message(
        message["content"], message["mode"], int(message["user"])
    )
...
def io_add_licence(message):
    result = request_add_licence_no_content(int(message["id"]), int(message["licence"]))
                    ...
                    user_active.append(
                        {
                            ...
                            "name": user["tele_name"],
                            "join_date": str(
                                timestamp_to_date(userr["join_date"] / 1000)
                            )[:19],
                ...
                user_active.append(
                    {
                        ...
                        "username": user["tele_username"],
                        "name": user["tele_name"],
                        "join_date": str(timestamp_to_date(userr["join_date"] / 1000))[
                            :19
                        ],
                        "licence_start": str(userr["full_licence_start"])[:19],
                        "licence": str(userr["full_licence"])[:19],
...
def html_symbol():
    ...
    for sym in DATA_SYMBOL:
        mini_sym = {"stt": sym["stt"], "symbol": sym["symbol"]}
...
def html_running_signal():
    ...
    list_group = []
    for group in DATA_LIST_GROUP:
        list_group.append({"id": group["group_id"], "name": group["name"]})
...
def html_closed_signal():
    ...
    url_args = {"date_from": date_from_str, "date_to": date_to_str}
...
def server_request_post():
    ...
    if result["option"] == "REQUEST_GET_RUN":
        return jsonify(
            {"token": {"alert": TOKEN_ALERT, "group": TOKEN_GROUP}, "admin": TELE_ADMIN}
        )
        ...
        retu = bot_request_trend(result["mode"], result["trend"], result["group_id"])
    return jsonify(retu)

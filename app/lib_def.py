...
def insert_port_tradingview(cursor, content):
    insert_user = """Insert INTO {} (opentime, signal) values(?, ?);""".format(
    ...
    for symbol in all_symbol:
        if symbol[1] == in_symbol.upper() and symbol[9] == "ON":
            ...
            if in_type == 0:
                ...
                result["sl"] = round_price(
                    s_entry_1, entry_avg * (1 - symbol[8] / c_de)
                )
                result["tp1"] = round_price(
                    s_entry_1, entry_avg * (1 + symbol[4] / c_de)
                )
                result["tp2"] = round_price(
                    s_entry_1, entry_avg * (1 + symbol[5] / c_de)
                )
                result["tp3"] = round_price(
                    s_entry_1, entry_avg * (1 + symbol[6] / c_de)
                )
                result["tp4"] = round_price(
                    s_entry_1, entry_avg * (1 + symbol[7] / c_de)
                )
                # new with tp5 tp6
                result["tp5"] = round_price(
                    s_entry_1, entry_avg * (1 + symbol[10] / c_de)
                )
                result["tp6"] = round_price(
                    s_entry_1, entry_avg * (1 + symbol[11] / c_de)
                )
            else:
                s_entry_2 = round_price(s_entry_1, s_entry_1 * (1 - symbol[3] / c_de))
                entry_avg = (s_entry_1 + s_entry_2) / 2
...
def get_form(cursor, mode, group_id, form_type):
    ...
    for form in all_form:
        if form[1] == mode and (
            (mode == "GROUP" and form[2] == group_id) or mode != "GROUP"
        ):
            if form_type == "CONTENT":
                result = form[3]
            elif form_type == "CONTENT_TP":
                result = form[4]
            elif form_type == "CONTENT_SL":
                result = form[5]
            elif form_type == "CONTENT_CL":
                result = form[6]
            elif form_type == "CONTENT_RP":
                result = form[7]
            elif form_type == "CONTENT_RP_CL":
                result = form[8]
            elif form_type == "CONTENT_RP_RUN":
                result = form[9]
            break
    if result is None:
...
def send_new_signal(cursor, signal):
    ...
    for group in all_group:
        if group[3] == "ON":  # status ON
            ...
            for active_symbol in all_active_symbol:
                if (
                    active_symbol[2] == "GROUP"
                    and active_symbol[4] == group[1]
                    and (active_symbol[3] == -1 or active_symbol[3] == signal["type"])
                    and active_symbol[1] == signal["symbol"]
                ):
                    ...
                    send_text = send_text.replace(
                        "[max_leverage]", str(signal["max_leverage"])
                    )
                    ...
                    if mess_id != -1:
                        insert_log = """Insert INTO {} (tele_id, opentime, mess_id, mode) values(?, ?, ?, ?);""".format(
                            "POST_SIGNAL"
                        )
    ...
    for user_alert in all_user_alert:
        # check active
        if (
            user_alert[4] > signal["opentime"]
            and user_alert[7] is not None
            and user_alert[7] == "ON"
        ):
            all_active_symbol = binance_check_table(cursor, "ACTIVE_SYMBOL")
            for active_symbol in all_active_symbol:
                ...
                if (
                    ...
                    send_text = send_text.replace("[take_profit_5]", str(signal["tp5"]))
                    send_text = send_text.replace("[take_profit_6]", str(signal["tp6"]))
                    mess_id = send_message_new_signal_user_alert(
                        user_alert[1], send_text
                    )
                    if mess_id != -1:
                        insert_log = """Insert INTO {} (tele_id, opentime, mess_id, mode) values(?, ?, ?, ?);""".format(
                            "POST_SIGNAL"
    ...
    for post_signal in all_post_signal:
        if post_signal[2] == signal["opentime"]:  # true opentime, check mode to send
            if post_signal[5] == "AUTO":
                ...
                content_cl = get_form(
                    cursor, post_signal[5], post_signal[1], "CONTENT_CL"
                )
                ...
                if post_signal[5] == "GROUP":
                    send_message_close_signal_group(
                        post_signal[1], send_text, post_signal[4]
                    )
                else:
                    send_message_close_signal_user_alert(
                        post_signal[1], send_text, post_signal[4]
                    )
    ...
    for post_signal in all_post_signal:
        if post_signal[2] == signal[1]:  # true opentime, check mode to send
            if post_signal[5] == "AUTO":
                ...
                content_tp = get_form(
                    cursor, post_signal[5], post_signal[1], "CONTENT_TP"
                )
                ...
                send_text = send_text.replace(
                    "[percent]", str(int(percent * 100) / 100) + "%"
                )
                send_text = send_text.replace(
                    "[percent_x2]", str(int(percent * 100 * 2) / 100) + "%"
                )
                # check mess_id
    ...
    for post_signal in all_post_signal:
        if post_signal[2] == signal[1]:  # true opentime, check mode to send
            if post_signal[5] == "AUTO":
                ...
                content_sl = get_form(
                    cursor, post_signal[5], post_signal[1], "CONTENT_SL"
                )

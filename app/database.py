...
def content_to_send_text(
    ...
    if symbol is not None:
        content = content.replace("[symbol]", str(symbol))
    if timeframe is not None:
        content = content.replace("[timeframe]", str(timeframe))
    if leverage is not None:
        content = content.replace("[max_leverage]", str(leverage))
    if entry_1 is not None:
        content = content.replace("[entry_1]", str(entry_1))
    if entry_2 is not None:
        content = content.replace("[entry_2]", str(entry_2))
    if sl is not None:
        content = content.replace("[stop_loss]", str(sl))
    if tp1 is not None:
        content = content.replace("[take_profit_1]", str(tp1))
    if tp2 is not None:
        content = content.replace("[take_profit_2]", str(tp2))
    if tp3 is not None:
        content = content.replace("[take_profit_3]", str(tp3))
    if tp4 is not None:
        content = content.replace("[take_profit_4]", str(tp4))
    if tp5 is not None:
        content = content.replace("[take_profit_5]", str(tp5))
    if tp6 is not None:
        content = content.replace("[take_profit_6]", str(tp6))
    if close_price is not None:
        content = content.replace("[close_price]", str(close_price))
    if tp is not None:
        content = content.replace("[tp]", str(tp))
    ...
    for i in range(len(DATA_SYMBOL)):
        if in_symbol == DATA_SYMBOL[i]["symbol"]:
            ...
            if in_status == "OFF":
                ...
                sql_delete_query = """DELETE from SYMBOL where symbol={}""".format(
                    in_symbol
                )
                ...
                in_max_level != DATA_SYMBOL[i]["max_leverage"]
                or in_entry_2 != DATA_SYMBOL[i]["entry_2"]
                or in_tp1 != DATA_SYMBOL[i]["tp1"]
                or in_tp2 != DATA_SYMBOL[i]["tp2"]
                or in_tp3 != DATA_SYMBOL[i]["tp3"]
                or in_tp4 != DATA_SYMBOL[i]["tp4"]
                or in_tp5 != DATA_SYMBOL[i]["tp5"]
                or in_tp6 != DATA_SYMBOL[i]["tp6"]
                or in_sl != DATA_SYMBOL[i]["sl"]
                or in_digit != DATA_SYMBOL[i]["digit"]
            ):
                is_modi = True
...
def request_add_form(mode, group_id, data):
    ...
    for i in range(len(DATA_FORM)):
        if DATA_FORM[i]["mode"] == mode and (
            mode == "ALERT" or (mode == "GROUP" and DATA_FORM[i]["id"] == group_id)
        ):
            is_new = False
            is_update = False
            conn = sqlite3.connect(database_binance_name)
    ...
    if mode in ["GROUP", "ALERT"]:
        ...
        if mode == "GROUP" and gr_number is not None:
            for i in range(len(DATA_LIST_GROUP)):
                if (
                    DATA_LIST_GROUP[i]["group_id"] == group_id
                    and DATA_LIST_GROUP[i]["number"] != gr_number
                ):
        ...
        if order_type in [-1, 0, 1, 99]:
            ...
            for i in range(len(DATA_ACTIVE_SYMBOL)):
                if DATA_ACTIVE_SYMBOL[i]["mode"] == mode and (
                    mode == "ALERT"
                    or (mode == "GROUP" and group_id == DATA_ACTIVE_SYMBOL[i]["id"])
                ):
                    true_change = False
                    set_type = None
                    if (
                        list_symbol is None
                        or DATA_ACTIVE_SYMBOL[i]["symbol"] not in list_symbol
                        and DATA_ACTIVE_SYMBOL[i]["type"] != 99
                    ):
                        true_change = True
                        set_type = 99
                    elif (
                        DATA_ACTIVE_SYMBOL[i]["type"] != order_type
                        and DATA_ACTIVE_SYMBOL[i]["symbol"] in list_symbol
                        and order_type != DATA_ACTIVE_SYMBOL[i]["type"]
                    ):
                        true_change = True
                        set_type = order_type
            ...
            if list_symbol is not None:
                for symbol in list_symbol:
                    ...
                    for i in range(len(DATA_ACTIVE_SYMBOL)):
                        if (
                            DATA_ACTIVE_SYMBOL[i]["mode"] == mode
                            and (
                                mode == "ALERT"
                                or (
                                    mode == "GROUP"
                                    and group_id == DATA_ACTIVE_SYMBOL[i]["id"]
                                )
                            )
                            and DATA_ACTIVE_SYMBOL[i]["symbol"] == symbol
                        ):
                            true_new = False
                            break
                    if true_new:
                        ...
                        DATA_ACTIVE_SYMBOL.append(
                            {
                                "stt": DATA_ACTIVE_SYMBOL[len(DATA_ACTIVE_SYMBOL) - 1][
                                    "stt"
                                ]
                                + 1,
                                "symbol": symbol,
            ...
            return {
                "status": False,
                "content": "Order Type {} not correct".format(order_type),
            }
    else:
    ...
    if mode in ["GROUP", "ALERT"]:
        if status in ["ON", "OFF"]:
            ...
            if tele_id == 0:
                ...
                content = "New status has been change for {} with id {}".format(
                    mode, tele_id
                )
            return {"status": True, "content": content}
        else:
...
def request_signal(in_signal):
    ...
    in_is_signal, in_type, in_symbol, in_tf, in_entry_1 = def_content_to_signal(
        in_signal
    )
    if in_is_signal:
        ...
        if is_new:
            for symbol in DATA_SYMBOL:
                if symbol["symbol"] == in_symbol and symbol["status"] == "ON":
                    ...
                    if in_type == 0:  # buy
                        entry_2 = round_price(
                            entry_1 * (1 - symbol["entry_2"] / c_de), digit
                        )
                    ...
                    for group in DATA_LIST_GROUP:
                        if group["status"] == "ON":
                            for active in DATA_ACTIVE_SYMBOL:
                                if (
                                    active["mode"] == "GROUP"
                                    and active["id"] == group["group_id"]
                                    and active["symbol"] == in_symbol
                                    and (
                                        active["type"] == -1
                                        or active["type"] == in_type
                                    )
                                ):
                                    # get group form
                                    is_form = False
                    ...
                    # ========================[ SEND TELEGRAM ]=========================
                    list_send_tele = []
                    result_send = {"total": 0, "complete": 0, "error": 0}
                    for gr in send_result_group:
                        if gr["number"] is not None and gr["number"] != -1:
                            try:
                                mess_id, send_err = send_signal(
                                    BOT_GROUP[gr["number"]], gr["id"], gr["content"]
                                )
                                result_send["total"] += 1
                                if mess_id != -1:
                                    list_send_tele.append(
                                        {
                                            "tele_id": gr["id"],
                                            "opentime": opentime,
                                            "mess_id": mess_id,
                    ...
                    if list_send_tele:
                        for sended in list_send_tele:
                            ...
                            DATA_POST_SIGNAL.append(
                                {
                                    "stt": DATA_POST_SIGNAL[len(DATA_POST_SIGNAL) - 1][
                                        "stt"
                                    ]
                                    + 1,
                    ...
                    send_text = (
                        "<code>" + str(in_signal) + "</code>\n"
                        "<b>Total:</b> {}\n"
                        "<b>Complete:</b> {}\n"
                        "<b>Time:</b> {}s".format(
                            result_send["total"], result_send["complete"], end
                        )
                    )
        ...
        if data_close:
            for data in data_close:
                ...
                if (
                    DATA_SIGNAL[data]["tp1_hit"] != "ON"
                    and DATA_SIGNAL[data]["tp2_hit"] != "ON"
                    and DATA_SIGNAL[data]["tp3_hit"] != "ON"
                    and DATA_SIGNAL[data]["tp4_hit"] != "ON"
                    and DATA_SIGNAL[data]["tp5_hit"] != "ON"
                    and DATA_SIGNAL[data]["tp6_hit"] != "ON"
                ):
                    # close order
                    # get post signal
                    url_check_m1 = "https://www.binance.com/fapi/v1/klines?symbol={}&interval=1m&limit=1".format(
                        DATA_SIGNAL[data]["symbol"].replace(".P", "")
                    )
                    try:
                ...
                update_trade = (
                    """UPDATE SIGNAL SET status=?, close_price=? WHERE opentime=?"""
                )
                ...
                for post_signal in list_close_post_signal:
                    ...
                    for form in DATA_FORM:
                        if form["mode"] == post_signal["mode"] and (
                            (
                                post_signal["mode"] == "GROUP"
                                and post_signal["tele_id"] == form["id"]
                            )
                            or post_signal["mode"] == "ALERT"
                        ):
                            user_content_cl = content_to_send_text(
                                form["content_cl"],
                    ...
                    if token_price != 0:
                        if post_signal["mode"] == "ALERT":
                            ...
                            for gr in DATA_LIST_GROUP:
                                if (
                                    gr["group_id"] == post_signal["tele_id"]
                                    and gr["number"] is not None
                                ):
                                    try:
                                        send_signal(
                                            BOT_GROUP[gr["number"]],
        ...
        if status is None:
            ...
            for i in range(len(DATA_LIST_GROUP)):
                if DATA_LIST_GROUP[i]["group_id"] == group_id:
                    ...
                    update_trade = (
                        """UPDATE LIST_GROUP SET name=?, status=? WHERE stt=?"""
                    )
                    info_update = (
                        DATA_LIST_GROUP[i]["name"],
                        new_status,
                        DATA_LIST_GROUP[i]["stt"],
                    )
                    cursor.execute(update_trade, info_update)
                    ...
                    result = {
                        "status": True,
                        "content": "Group {} status has been updated".format(
                            DATA_LIST_GROUP[i]["name"]
                        ),
                    }
                    break
            ...
            is_url, is_member, gr_id, gr_title, gr_username = get_channel_info(
                link="", channel_id=group_id
            )
            """
            ...
            for i in range(len(DATA_LIST_GROUP)):
                if DATA_LIST_GROUP[i]["group_id"] == gr_id:
                    ...
                    if (
                        ...
                        or DATA_LIST_GROUP[i]["status"] != status
                    ):
                        conn = sqlite3.connect(database_binance_name)
                        cursor = conn.cursor()
                        ...
                        result = {
                            "status": True,
                            "content": "Group {} has been updated".format(gr_title),
                        }
                        break
            if is_new:
                if status is None:
                ...
                result = {
                    "status": True,
                    "content": "Group {} has been added".format(gr_title),
                }
...
def request_update_content(mode, group_id, content_type, content):
    if mode in ["GROUP", "ALERT"] and content_type in [
        "content",
        "content_tp",
        "content_sl",
        "content_cl",
        ...
        if not true_set:
            ...
            for i in range(len(DATA_FORM)):
                if (
                    mode == DATA_FORM[i]["mode"] == "GROUP"
                    and DATA_FORM[i]["id"] == group_id
                ) or mode == DATA_FORM[i]["mode"] == "ALERT":
                    is_new = False
                    conn = sqlite3.connect(database_binance_name)
                    cursor = conn.cursor()

                    update_trade = """UPDATE FORM SET {}=? WHERE stt=?""".format(
                        content_type
                    )
            ...
            if is_new:
                content_z = (
                    ...
                    content_sl
                ) = content_cl = content_rp = content_rp_cl = content_rp_run = "Empty"
                if content_type == "content":
                    content_z = content
                elif content_type == "content_tp":
                    content_tp = content
                elif content_type == "content_sl":
                    content_sl = content
                elif content_type == "content_cl":
                    content_cl = content
                elif content_type == "content_rp":
                    content_rp = content
                elif content_type == "content_rp_cl":
                    content_rp_cl = content
                elif content_type == "content_rp_run":
                    content_rp_run = content
...
def request_update_symbol(
    symbol, leverage, entry_2, tp1, tp2, tp3, tp4, tp5, tp6, tp7, sl, digit
):
    result = {}
    is_update = False
    for i in range(len(DATA_SYMBOL)):
        if DATA_SYMBOL[i]["symbol"] == symbol:
            ...
            if (
                ...
                conn.commit()
                conn.close()
                result = {"status": True, "content": "Symbol {} updated".format(symbol)}
            else:
...
def request_confirm_licence(ordertime, licence):
    ...
    for i in range(len(DATA_USER_BUY_LICENCE)):
        if (
            DATA_USER_BUY_LICENCE[i]["ordertime"] == ordertime
            and DATA_USER_BUY_LICENCE[i]["confirm"] == 0
        ):
            if licence == -1:
                ...
                update_trade = (
                    """UPDATE USER_BUY_LICENCE SET confirm=? WHERE ordertime=?"""
                )
                ...
                for j in range(len(DATA_USER_ALERT)):
                    if (
                        DATA_USER_ALERT[j]["tele_id"]
                        == DATA_USER_BUY_LICENCE[i]["tele_id"]
                    ):
                        if (
                            DATA_USER_ALERT[j]["licence"] == 0
                            or DATA_USER_ALERT[j]["licence"] < ts_now
                        ):
                        ...
                        DATA_USER_ALERT[j]["full_licence"] = str(
                            timestamp_to_date(new_licence / 1000)
                        )[:19]
                        DATA_USER_ALERT[j]["full_licence_start"] = str(
                            timestamp_to_date(ts_now / 1000)
                        )[:19]
...
def request_add_licence_no_content(tele_id, licence):
    ...
    for i in range(len(DATA_USER_ALERT)):
        if DATA_USER_ALERT[i]["tele_id"] == tele_id:
            ...
            result = {
                "status": True,
                "content": "Done, new licence: "
                + str(timestamp_to_date(DATA_USER_ALERT[i]["licence"] / 1000))[:19],
            }
            break
    if result is None:
...
def request_send_message(content, mode, user_id):
    ...
    for user in DATA_USER_ALERT:
        ...
        if (
            (mode == "active" and is_active)
            or (mode == "inactive" and not is_active)
    ...
    return {"status": True, "content": text_rs}
...
def set_default_symbol():
    ...
    for info in info_of_symbol:
        ...
        result.append({"status": add_result["status"], "symbol": info["symbol"]})
    return result
...
def hit_sl_tp(opentime, hit_data):
    ...
    for i in range(len(DATA_SIGNAL)):
        if (
            DATA_SIGNAL[i]["opentime"] == opentime
            and DATA_SIGNAL[i]["symbol"] == hit_data["symbol"]
        ):
            true_send = None
            tp = -1
            if hit_data["tp6_hit"] == "ON" and DATA_SIGNAL[i]["tp6_hit"] == "OFF":
                true_send = calculator_percent(
                    DATA_SIGNAL[i]["ordertype"],
                    DATA_SIGNAL[i]["entry_1"],
                    DATA_SIGNAL[i]["tp6"],
                )
                DATA_SIGNAL[i]["tp1_hit"] = DATA_SIGNAL[i]["tp2_hit"] = DATA_SIGNAL[i][
                    "tp3_hit"
                ] = DATA_SIGNAL[i]["tp4_hit"] = DATA_SIGNAL[i]["tp5_hit"] = DATA_SIGNAL[
                    i
                ][
                    "tp6_hit"
                ] = "ON"
            ...
            if true_send is not None:
                for post in DATA_POST_SIGNAL:
                    if post["opentime"] == opentime:
                        for form in DATA_FORM:
                            if post["mode"] == form["mode"] and (
                                post["mode"] == "ALERT"
                                or (
                                    post["mode"] == "GROUP"
                                    and post["tele_id"] == form["id"]
                                )
                            ):
                                if true_send >= 0:
                                    content_tp = content_to_send_text(
                                        form["content_tp"],
                                        sl=DATA_SIGNAL[i]["sl"],
                                ...
                                if true_send_tele is None:
                                    ...
                                    true_send_tele += 1
                                if post["mode"] == "ALERT":
                                    send_result, send_err = send_signal(
                                        BOT_ALERT,
                                        post["tele_id"],
                                        content_tp,
                                        post["mess_id"],
...
def send_signal_report(
    content_type, target, date_from, date_to, mode, send_end, end_content
):
    result = {}
    if content_type in ["content_rp", "content_rp_run", "content_rp_cl"]:
        if (
            len(target.split(":")) == 2
            and target.split(":")[0] in ["alert", "group"]
            and (
                (
                    target.split(":")[0] == "alert"
                    and target.split(":")[1] in ["active", "inactive"]
                )
            ...
            try:
                from_date = datetime.strptime(
                    date_from + " 00:00:00", "%Y-%m-%d %H:%M:%S"
                )
            ...
            if ts_to > ts_from != 0:
                content = "Empty"
                for form in DATA_FORM:
                    if form["mode"] == target.split(":")[0].upper() and (
                        form["mode"] == "ALERT"
                        or (
                            form["mode"] == "GROUP"
                            and form["id"] == int(target.split(":")[1])
                        )
                    ):
                ...
                for iii in range(len(DATA_POST_SIGNAL)):
                    ...
                    if post["mode"] == target.split(":")[0].upper() and (
                        post["mode"] == "ALERT"
                        or (
                            post["mode"] == "GROUP"
                            and post["tele_id"] == int(target.split(":")[1])
                        )
                    ):
                ...
                for signal in DATA_SIGNAL:
                    if (
                        ts_from <= signal["opentime"] <= ts_to
                        and (
                            (
                                signal["status"] == "OFF"
                                and content_type in ["content_rp", "content_rp_cl"]
                            )
                            or (
                                signal["status"] == "ON"
                                and content_type in ["content_rp", "content_rp_run"]
                            )
                        )
                        and signal["opentime"] in LIST_POST_SIGNAL_DATA
                    ):
                        total_order += 1
                        if signal["tp6_hit"] == "ON":
                            end_price = signal["tp6"]
                        elif signal["tp5_hit"] == "ON":
                            end_price = signal["tp5"]
                        elif signal["tp4_hit"] == "ON":
                            end_price = signal["tp4"]
                        elif signal["tp3_hit"] == "ON":
                            end_price = signal["tp3"]
                        elif signal["tp2_hit"] == "ON":
                            end_price = signal["tp2"]
                        elif signal["tp1_hit"] == "ON":
                            end_price = signal["tp1"]
                        elif signal["sl_hit"] == "ON":
                            end_price = signal["sl"]
                        elif signal["close_price"] != 0:
                            end_price = signal["close_price"]
                        else:
                            end_price = -9999
                        ...
                        if end_price == -9999:
                            ...
                            # if end_price > 0:
                            #     total_win += 1
                            percent = calculator_percent(
                                signal["ordertype"], signal["entry_1"], end_price
                            )
                ...
                if total_percent < 0:
                    content_3 = content_3.replace(
                        "[total_percent]", str(int(total_percent * 100) / 100) + "%"
                    )
                    content_3 = content_3.replace(
                        "[total_percent_2x]",
                        str(int(total_percent * 2 * 100) / 100) + "%",
                    )
                    content_3 = content_3.replace(
                        "[this_pnl]", str(int(total_percent * 100) / 100) + "%"
                    )
                else:
                    content_3 = content_3.replace(
                        "[total_percent]",
                        "+" + str(int(total_percent * 100) / 100) + "%",
                    )
                    content_3 = content_3.replace(
                        "[total_percent_2x]",
                        "+" + str(int(total_percent * 2 * 100) / 100) + "%",
                ...
                content_3 = content_3.replace(
                    "[total_loss]", str(int(total_order - total_win))
                )
                ...
                if mode == "TEST":
                    mess_id, send_err = send_signal(
                        BOT_ALERT, LOG_TEST_REPORT, content_send
                    )
                    result = {
                        "status": True,
                    ...
                    result = {"status": True, "content_tele": content_send}
                    ...
                    for user in DATA_USER_ALERT:
                        if (user_type == "active" and user["licence"] > ts_now) or (
                            ...
                            mess_id, send_err = send_signal(
                                BOT_ALERT, user["tele_id"], content_send
                            )
                            total_send += 1
                            if mess_id != -1:
                                total_complete += 1
                    ...
                    report_text = (
                        ...
                        "Complete: {}<br>"
                        "Time: {}s".format(
                            total_send, total_complete, str(timeend - timest)
                        )
                    )
                    result = {
                        "status": True,
                    ...
                    for group in DATA_LIST_GROUP:
                        if group["group_id"] == group_id:
                            try:
                                mess_id, send_err = send_signal(
                                    BOT_GROUP[group["number"]], group_id, content_send
                                )
                                if mess_id != -1:
                                    send_res = "Send completed ! GROUP: {}".format(
                                        group["name"]
                                    )
                                else:
                                    send_res = send_err
            ...
            result = {"status": False, "content": "Target not correct"}
    else:
        result = {"status": False, "content": "Content type not found"}
    return result
...
def bot_request_user(tele_id, username, name):
    ...
    for i in range(len(DATA_USER)):
        if DATA_USER[i]["tele_id"] == tele_id:
            ...
            if is_edit:
                ...
                update_trade = (
                    """UPDATE USER SET tele_username=?, tele_name WHERE tele_id=?"""
                )
    ...
    if is_new:
        ...
        if username == "":
            ...
            send_text += (
                "Name: <a href='https://t.me/" + username + "'>" + name + "</a>\n"
            )
...
def bot_request_trend(mode, new_trend, group_id):
    ...
    if mode in ["ALERT", "GROUP"] and new_trend in [-1, 0, 1, 99]:
        for i in range(len(DATA_ACTIVE_SYMBOL)):
            ...
            if DATA_ACTIVE_SYMBOL[i]["mode"] == mode and (
                mode == "ALERT"
                or (mode == "GROUP" and DATA_ACTIVE_SYMBOL[i]["id"] == group_id)
            ):
                is_edited = True
